...
This project includes a production-ready [Model Context Protocol](https://modelcontextprotocol.io) server that exposes website content to AI clients (Claude Desktop, Cursor, VS Code).

```bash
npm run mcp:dev      # Start MCP server (stdio)
npm run mcp:test     # Run content index smoke test
npm run mcp:inspect  # Interactive MCP Inspector
```

See **[mcp/README.md](mcp/README.md)** for full setup, tool reference, and client configuration.
